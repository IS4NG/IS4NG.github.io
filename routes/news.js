...
router.get('/news', function(요청,응답){
    console.log(요청.user);
    var searchquery = { board : 'news'};
    db.collection('posting').find(searchquery).toArray(function(에러, 결과){
      console.log(결과);
        응답.render('./news/newsa.ejs', { 글목록 : 결과});
    });
  })

  router.get('/news/create',  로그인했니,function(요청,응답){
    응답.render('./news/create.ejs')
  })
  router.post('/news/post',로그인했니,function(요청,응답){
    let today = new Date();
  ...
  router.get('/news/edit/:id', 로그인했니,function(요청, 응답){
    ...
    db.collection('posting').findOne(수정할데이터, function(에러, 결과){
        ...
        응답.render('news/edit.ejs', {data : 결과} )
      }
    })
  });
  router.post('/news/edit/put',function(요청,응답){
    let today = new Date();
    let year = today.getFullYear(); 
    let month = today.getMonth() + 1
    let date = today.getDate();
