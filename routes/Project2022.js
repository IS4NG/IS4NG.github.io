...
router.get('/Project/2022', function(요청,응답){
    console.log(요청.user);
    var searchquery = { board : 'Project2022'};
    db.collection('posting').find(searchquery).toArray(function(에러, 결과){
      console.log(결과);
        응답.render('./2022p/2022a.ejs', { 글목록 : 결과});
    });
  })

  router.get('/Project/2022/create',  로그인했니,function(요청,응답){
    응답.render('./2022p/create.ejs')
  })
  router.post('/Project/2022/post',로그인했니,function(요청,응답){
    let today = new Date();
  ...
  router.get('/Project/2022/detail/:id', function(요청, 응답){
    db.collection('posting').findOne({ _id : parseInt(요청.params.id) }, function(에러, 결과){
        응답.render('2022p/2022b.ejs', {data : 결과} )
    })
  });
